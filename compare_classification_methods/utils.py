...
# Utility to resize images to the target size using Pillow
def resize_images(X, target_size):
    X_resize = []
    for img in X:
        pil_img = Image.fromarray(img)  # Convert numpy array to PIL image
        resized_img = pil_img.resize(target_size, Image.ANTIALIAS)  # Resize with Pillow
        X_resize.append(np.array(resized_img))  # Convert back to numpy array
    return np.array(X_resize)


# CIFAR-10 loading function (add this if needed in utils)
def load_cifar10_data(data_dir, target_size=(24, 24), num_classes=10):
    def unpickle(file):
    ...
    x_test = test_batch[b'data'].reshape(-1, 3, 32, 32).astype("float32")
